...
async function fetchRestaurantsWithPhoneNumbers(apiKey) {
    ...
    rl.question("Enter the address: ", async (address) => {
        ...
        rl.question("Enter the search radius in meters (e.g., 1000): ", async (radiusInput) => {
            ...
            // Asking for the JSON filename
            rl.question("Enter the JSON filename (e.g., 'restaurants.json'): ", async (jsonFileName) => {
                if (!jsonFileName) {
                    console.error("JSON filename is required.");
                    rl.close();
                    return;
                }
                ...
                try {
                    ...
                    // If results are available, save to JSON
                    if (allFoodDetails.length === 0) {
                        ...
                        fs.writeFileSync(jsonFilePath, JSON.stringify(allFoodDetails, null, 2), "utf-8");
